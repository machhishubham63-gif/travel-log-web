...
export default function App() {
  ...
  const icons = {
    ...
    persons: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="20" height="20"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
  };
    ...
    <div style={{ maxWidth: "600px", margin: "0 auto", fontFamily: "system-ui, -apple-system, sans-serif", backgroundColor: "var(--bg-main)", minHeight: "100vh", display: "flex", flexDirection: "column", color: "var(--text-main)", transition: "background-color 0.3s" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", backgroundColor: "var(--bg-surface)", position: "sticky", top: 0, zIndex: 100, borderBottom: "1px solid var(--border-light)" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          {/* NEW: PERSONS BUTTON */}
          <button onClick={() => setActiveTab("persons")} style={{ padding: "8px", backgroundColor: "var(--bg-input)", color: activeTab === "persons" ? "var(--accent-blue)" : "var(--text-muted)", border: "1px solid var(--border-strong)", borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            {icons.persons}
          </button>
          ...
          <button onClick={handleLogout} style={{ padding: "8px 16px", backgroundColor: "var(--accent-red-bg)", color: "var(--accent-red)", border: "1px solid var(--accent-red)", borderRadius: "20px", fontSize: "14px", fontWeight: "800", cursor: "pointer", marginLeft: "4px" }}>
            Logout
          </button>
        </div>
      </div>
